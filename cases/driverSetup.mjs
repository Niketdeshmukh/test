...
// Function to restart the ADB server
function restartADB() {
  console.log('Restarting ADB server...');
  try {
    execSync('adb kill-server'); // Kill any running ADB server
    execSync('adb start-server'); // Restart the ADB server
    console.log('ADB server restarted successfully.');
  } catch (error) {
    console.error('Error restarting ADB server:', error.message);
    throw error;
  }
}
